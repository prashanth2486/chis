...
def render():
    ...
    with tab1:
        st.subheader("System Metrics & Health")
        try:
            ...
            st.error("Could not reach backend for analytics.")
    ...
    with tab2:
        ...
        try:
            ...
            if res_users.status_code == 200:
                ...
                st.error("Could not fetch user directory.")
        except Exception as e:
            st.error("Failed to connect to backend server.")

    with tab3:
        st.header("Pattern Prediction & Mining (Eclat)")
        st.markdown("""
        ...
        with col1:
            min_support = st.slider("Minimum Support", min_value=0.01, max_value=1.0, value=0.04, step=0.01, help="Minimum threshold for itemset support (percentage of transactions containing the itemset).")
        with col2:
            min_confidence = st.slider("Minimum Confidence", min_value=0.1, max_value=1.0, value=0.50, step=0.05, help="Minimum threshold for rule confidence.")
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if rules:
                                ...
                                df_rules = df_rules.rename(columns={"lhs": "Antecedents (LHS)", "rhs": "Consequents (RHS)", "confidence": "Confidence"})
                        ...
                        st.error("Failed to connect to the backend server. Make sure the FastAPI backend is running on http://localhost:8000")
